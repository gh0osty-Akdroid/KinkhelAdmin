...
const VoucherCategory = () => {
    ...
    useEffect(() => {
        ...
        Title("Merchant Voucher Categories")
        adminTokenUrl().get(`/voucher-categories/${id}`).then((res) => {
    ...
    const handleVoucherSearch = (e) => {
        e.preventDefault()
        setLoading(true)
        setTimeout(() => setLoading(false), 1000)
        adminTokenUrl().get(`/voucher-categories/${id}?input=${input}`).then((res) => {
            setData(res?.data?.data?.data);
            setPages(res?.data?.data?.totalPages);
            setRefresh(false)
            setOpened(false)
    ...
    const handleUpdate = (e) => {
        const body = { "validity": validity, "active": active, "id": uid, "batch": batch }
            ...
            <div className="m-2 md:m-10 mt-18 p-2 md:p-10 dark:text-gray-200 dark:bg-secondary-dark-bg rounded-3xl">
                ...
                <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
                    <div className="flex justify-center text-center pb-4 bg-white dark:bg-gray-900 p-5">
                        <form onSubmit={handleVoucherSearch}>
