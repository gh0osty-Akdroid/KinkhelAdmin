...
const VoucherList = () => {
  ...
  useEffect(() => {
    ...
    adminTokenUrl().get(`/voucher-lists/${uid}/${id}?page=${page}&enable=${enable}&batch=${batchData}`).then((res) => {
  ...
  useEffect(() => {
    adminTokenUrl().get(`/voucher-category/${uid}`).then((res) => {
  ...
  const handlePrint = () => {
    let batchData = batch === "all" ? null : batch
    adminTokenUrl().get(`/download-voucher-lists-admin/${uid}?batch=${batchData}&merchant=${id}`,).then((res) => {
  ...
  const Update = () => {
    ...
    if (confirm("Do You want to update?")) {
      adminTokenUrl().put('/voucher-lists-admin', { items: items, from: from, to: to, active: active, batch: batch_, id: uid, merchant_id :id }).then((res) => {
                      ...
                      <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" htmlFor={i}>
                        <td className="py-4 px-6">
                          <div className="flex items-center">
                            <Checkbox onChange={() => Select(e.id)} on className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
