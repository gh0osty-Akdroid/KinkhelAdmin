...
import { adminTokenUrl, adminUrl } from '../../Urls';
...
function index() {
  ...
  useEffect(() => {
    ...
    adminTokenUrl().get(`/merchants?site=${site}`).then((res) => {
  ...
  const handleMerchantSearch =(e)=>{
    ...
    setLoading(true)
    setTimeout(() => setLoading(false), 1000)
    adminTokenUrl().get(`/merchants?site=${site}&input=${input}`).then((res)=>{ 
      setData(res?.data?.data?.data);
      setPages(res?.data?.data?.totalPages)
    }).catch((err)=>{
      ErrorHandler(err)
    ...
    <div className="m-2 md:m-10 mt-18 p-2 md:p-10 dark:text-gray-200 dark:bg-secondary-dark-bg rounded-3xl">
      ...
      <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
        <div className="flex justify-center text-center pb-4 bg-white dark:bg-gray-900 p-5">
          <form onSubmit={handleMerchantSearch}>
            ...
            <div className="relative">
              ...
              <input type="text" id="table-search-users" value={input} className="block p-2 pl-10 w-80 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search Merchants Name or Phone or Merchant Code" onChange={(e)=>setInput(e.target.value)} />
            </div>
          </form>
