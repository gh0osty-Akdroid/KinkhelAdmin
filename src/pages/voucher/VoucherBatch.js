...
const VoucherBatch = () => {
  ...
  const handleEnable = (active, batch) => {
    adminTokenUrl().put(`voucher-lists-admin?active=${active}&batch=${batch}&id=${id}`, { merchant_id: null }).then((res) => {
      SuccessNotification({ title: "Updated!", message: "The Batch has been updated." })
      setRefresh(true)
    }).catch((err) => {
      ErrorHandler(err)
    })
          ...
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" >
                ...
                <td className="py-4 px-6">
                  <Button.Group>
                    <Button variant='outline' disabled className='text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500' onClick={() => handleEnable(true, e.value)}>Enable</Button>
