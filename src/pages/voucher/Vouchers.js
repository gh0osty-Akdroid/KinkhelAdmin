...
import { Pagination, Modal, Grid, Switch, MultiSelect, Button } from '@mantine/core';
...
const Vouchers = () => {
  ...
  useEffect(() => {
    ...
    adminTokenUrl().get(`/voucher-categories?page=${page}`).then((res) => {
  ...
  const handleSearch = (e) =>{
    ...
    setLoading(true)
    setTimeout(() => setLoading(false), 1000)
    adminTokenUrl().get(`/voucher-categories?input=${input}`).then((res) => {
        setData(res?.data?.data?.data);
        setPages(res?.data?.data?.totalPages);
        setRefresh(false)
        setOpened(false)
