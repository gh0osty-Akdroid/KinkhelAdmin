...
const VoucherList = () => {
  ...
  useEffect(() => {
    ...
    adminTokenUrl().get(`/voucher-lists-admin/${id}?page=${page}&enable=${enable}&batch=${batchData}`).then((res) => {
  ...
  useEffect(() => {
    adminTokenUrl().get(`/voucher-category/${id}`).then((res) => {
  ...
  const handlePrint = () => {
    let batchData = batch === "all" ? null : batch
    adminTokenUrl().get(`/download-voucher-lists-admin/${id}?batch=${batchData}&merchant=${null}`,).then((res) => {
  ...
  const Select = (e) => {
    ...
    if (items.length > 0) { setButton(true) } else return setButton(false)
  }
  ...
  const Update = () => {
    ...
    if (confirm("Do You want to update?")) {
      adminTokenUrl().put(`/voucher-lists-admin`, { items: items, from: from, to: to, active: active, batch: batch_, id: id, merchant_id:null }).then((res) => {
    ...
    <>{loading ? <Loadings/>: <>
      <div className="m-2 md:m-10 mt-18 p-2 md:p-10 dark:text-gray-200 dark:bg-secondary-dark-bg rounded-3xl">
        ...
        <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
          <div className="flex text-center pb-4 bg-white dark:bg-gray-900 p-5">
                      ...
                      <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" htmlFor={i}>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center">
                            <input type={"checkbox"} id={i} onChange={() => Select(e.id)} on className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
